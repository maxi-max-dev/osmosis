...
[OpenAI Economic Research](https://openai.com/index/how-agents-are-transforming-work) reports 137x growth in non-developer individual Codex users from August 2025 to early June 2026, and that 70.2% of sampled individual users made at least one Codex request estimated to exceed one hour of human work. OpenAI describes those model-estimated task horizons as directional rather than exact. These figures measure adoption and model-estimated task horizons, not learning or comprehension. For comprehension evidence, in [Anthropic's randomized study](https://www.anthropic.com/research/AI-assistance-coding-skills), 52 mostly junior software engineers learning a new Python library scored 17% lower on an immediate comprehension quiz when using AI assistance than when coding by hand. Osmosis uses the agent's wait time to turn the work itself into short retrieval questions, so the owner builds a usable model of the technology while the project moves forward.
