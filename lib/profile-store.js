...
async function readLockInfo(lockPath, { fsApi = fs } = {}) {
  try {
    ...
    try {
      ...
      // An interrupted lock write is unsafe to trust or reclaim. Valid older
      // records without a timestamp may still use the file mtime below.
    }
    return {
      lockPath,
...
async function isLockInfoStale(info, { now = defaultNowMs, staleMs = 30_000, isProcessAlive = processIsAlive } = {}) {
  ...
  if (!identity) {
    // A malformed lock does not give us a holder that we can prove dead.
    // Keeping it is deliberately conservative; the caller can surface a
    // timeout rather than risk evicting a writer during an interrupted I/O.
    return false;
  }
  const acquiredAt = lockAcquiredAt(info);
...
/**
 * Atomically detach a particular lock record, then re-read the moved inode.
 * If another process replaced the pathname between our read and rename, the
 * token check fails and we restore it without clobbering a newer owner.
 */
async function moveLockAsideIfOwned(lockPath, expectedRecord, {
  fsApi = fs,
  label = 'reclaim',
} = {}) {
  const beforeMove = await readLockInfo(lockPath, { fsApi });
  if (!hasSameLockIdentity(beforeMove?.record, expectedRecord)) {
    return null;
  }
  ...
  if (!hasSameLockIdentity(moved?.record, expectedRecord)) {
    await restoreMovedLock(lockPath, movedPath, { fsApi });
    return null;
  }
...
async function reclaimStaleLock(lockPath, expectedInfo, options = {}) {
  const { fsApi = fs } = options;
  const expectedRecord = expectedInfo?.record;
  if (!await isLockInfoStale(expectedInfo, options)) {
    return false;
  }
  const moved = await moveLockAsideIfOwned(lockPath, expectedRecord, { fsApi, label: 'reclaim' });
  if (!moved) {
    return false;
  }
...
async function acquireProfileLock({
  ...
  while (true) {
    ...
    try {
      ...
      // O_EXCL establishes a path-level candidate, not permanent ownership:
      // stale recovery or a replacement race may have moved it before we
      // return. Require our pid/token to still be at the public pathname.
      const current = await readLockInfo(lockPath, { fsApi });
      ...
      return {
        ...
        async release() {
          ...
          try {
            ...
            // Never unlink the public path after a separate read: a later
            // owner could have taken it. The release helper moves and verifies
            // our token before removing anything.
            await removeOwnedLock(lockPath, record, { fsApi });
          }
        },
      };
      ...
      // If a completed write left our record behind, remove only that exact
      // token. A malformed partial record is intentionally retained rather
      // than risk deleting a file another process has since installed.
      await removeOwnedLock(lockPath, record, { fsApi });
      throw error;
    }
  }
