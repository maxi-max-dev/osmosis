...
function normalizeStudioState(value, cards = []) {
  ...
  return {
    ...
    ready_card: readyCard && readyCard.card_id !== currentCardId ? readyCard : null,
...
function studioSnapshot(studio, cards = []) {
  const normalized = studio && typeof studio === 'object' && !Array.isArray(studio)
    ? studio
    : defaultStudioState();
  const currentId = boundedString(normalized.current_card_id, 128);
  const current = currentId
    ? (Array.isArray(cards) ? cards.find((card) => card?.card_id === currentId) || null : null)
    : null;
  const ready = studioReadyCard(normalized, cards);
  return {
    current: current ? cardForClient(current) : null,
    next_ready: Boolean(ready),
    waiting: studioWaiting(normalized, cards),
  };
}
