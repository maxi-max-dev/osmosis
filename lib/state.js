...
function createPersistence(config, {
  ...
  async function commitCards(cards, studio) {
    if (typeof beforeCardsWrite === 'function') {
      await beforeCardsWrite({ cards, studio });
    }
    ...
    try {
      ...
      cardsRevision = await saveCards(config, cards, studio);
  ...
  return {
    saveCards(cards, studio) {
      const queued = enqueue(cardsWrite, async () => {
        ...
        await commitCards(cards, rememberedStudio);
      });
    ...
    saveStudio(studio, cards) {
      const queued = enqueue(cardsWrite, async () => {
        ...
        loadedStudio = true;
        await commitCards(existingCards, rememberedStudio);
      });
      cardsWrite = queued.tail;
      return queued.next;
    },
