...
const { normalizePresentation } = require('./presentation-projection');
...
async function readJsonWithRevision(filePath, fallback) {
  try {
    const text = await fs.readFile(filePath, 'utf8');
    return { revision: documentRevision(text), value: JSON.parse(text) };
  } catch (error) {
    if (error && error.code === 'ENOENT') {
      return { revision: null, value: fallback };
    }
    throw error;
  }
}
...
function defaultStudioState() {
  return {
    ...
    // lesson preparation.
    progress: null,
    // Display-only activity truth for the three-step strip. It is persisted
    // alongside Studio state solely so a reconnect resumes the honest phase;
    // no scheduler or card transition reads it back as an instruction.
...
function normalizeStudioReport(value) {
  ...
  return {
    ...
    ...(Array.isArray(value.activity_epoch_ids)
      ...
      : {}),
    // This is an opaque installation-HMAC lookup key only.  It is kept with
    // a durable candidate so a restart cannot silently detach an ambient
    // lesson from its local-only presentation badge; raw session ids and
    // prompt text are rejected by construction and never enter Studio state.
    ...(typeof value.conversation_id === 'string' && /^[a-f0-9]{24,64}$/i.test(value.conversation_id)
...
function studioSnapshot(studio, cards = []) {
  ...
  return {
    ...
    presentation: normalizedState.presentation,
...
async function loadProjectState(config, sharedStrengths = null) {
  ...
  const loadedCards = await readJsonWithRevision(path.join(config.stateDir, 'cards.json'), { cards: [] });
  const cardDocument = loadedCards.value;
  ...
  return {
    cards,
    cards_revision: loadedCards.revision,
...
async function saveCards(config, cards, studio, { writeEpoch = null } = {}) {
  const document = { cards };
  if (studio !== undefined) {
    document.studio = normalizeStudioState(studio, cards);
  }
  if (typeof writeEpoch === 'string' && writeEpoch) {
    document.write_epoch = writeEpoch;
  }
  const serialized = await writeJsonAtomic(path.join(config.stateDir, 'cards.json'), document);
  return documentRevision(serialized);
}
...
function staleCardsWriteError() {
  const error = new Error('Cards changed after this owner loaded them; refusing a stale project-state write.');
  error.code = 'ESTALEWRITE';
  return error;
}

function ownerWriteError() {
  const error = new Error('This process no longer owns the project-state write lease.');
  error.code = 'EOWNERLEASE';
  return error;
}

function createPersistence(config, {
  profileStore = null,
  initialCardsRevision,
  beforeCardsWrite = null,
  canCommit = () => true,
  cardsLockOptions = {},
  writeEpoch = null,
} = {}) {
  ...
  // An owner carries the exact cards.json revision it hydrated. The lock
  // serializes cross-process commit attempts; the revision check rejects an
  // older owner after a takeover has already written a newer document.
  let cardsRevision = initialCardsRevision;
  ...
  async function commitCards(cards, studioForWrite) {
    if (typeof beforeCardsWrite === 'function') {
      await beforeCardsWrite({ cards });
    }
    const cardsPath = path.join(config.stateDir, 'cards.json');
    const lock = await acquireProfileLock({
      lockPath: `${cardsPath}.lock`,
      ...cardsLockOptions,
    });
    try {
      // `canCommit` may re-read a durable owner fence, so it is deliberately
      // awaited only after this writer has acquired the cross-process cards
      // lock. A former owner cannot pass an early check, wait on the lock,
      // then write with an old revision after a takeover publishes its epoch.
      if (!await canCommit()) {
        throw ownerWriteError();
      }
      const current = await readJsonWithRevision(cardsPath, { cards: [] });
      // Compatibility callers without a hydration revision begin from the
      // document present at their first locked write. Owner channels always
      // supply their hydration revision, so they cannot overwrite a takeover.
      if (cardsRevision === undefined) {
        cardsRevision = current.revision;
      }
      if (current.revision !== cardsRevision) {
        // A broker has one random epoch for all of its legitimate in-process
        // persistence work. A separate persistence instance can therefore
        // commit first without turning this still-current owner into a false
        // stale writer. A different epoch is a prior/other owner and remains
        // a hard refusal rather than a reload that could overwrite it.
        if (typeof writeEpoch === 'string' && writeEpoch && current.value?.write_epoch === writeEpoch) {
          cardsRevision = current.revision;
        } else {
          throw staleCardsWriteError();
        }
      }
      // Resolve the live Studio object only after the lock and any same-owner
      // rebase. Sibling current-owner work can therefore not make this write
      // serialize an older pre-await Studio snapshot over newer state.
      const studio = typeof studioForWrite === 'function' ? studioForWrite() : studioForWrite;
      cardsRevision = await saveCards(config, cards, studio, { writeEpoch });
  ...
  return {
    ...
    saveStudio(studio, cards) {
      const queued = enqueue(cardsWrite, async () => {
        ...
        loadedStudio = true;
        await commitCards(existingCards, () => {
    ...
    setCardsRevision(revision) {
      cardsRevision = revision;
    },
  };
}
...
function cardForClient(card) {
  // Answer receipts are durable operator recovery evidence, not browser
  // lesson data. Keep their stable id local to cards.json/receipt storage.
  const { correct_index, explanation, answer_receipt, ...visibleCard } = card;
