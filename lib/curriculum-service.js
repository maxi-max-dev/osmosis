...
function unansweredCount(cards, studio = null) {
  // A generated Next card lives in `studio.ready_card`, not `cards`, until
  // the learner explicitly promotes it. It therefore naturally stays out of
  // the five-question historical cap.
  return (Array.isArray(cards) ? cards : []).filter(
    (card) => !card.state?.answered,
  ).length;
}

function createCurriculumService({ config, hub, provider, state, treeService, canCommit = () => true, clock = () => Date.now(), sleep = delay }) {
  ...
  function inactiveSkip() {
    return {
      enabled: true,
      skip: {
        state: 'owner-inactive',
        message: 'Osmosis is reconnecting its local owner.',
      },
    };
  }
  ...
  async function markDirectConcept(report, concepts) {
    ...
    if (direct) {
      return treeService.markSurfaced(direct.concept_id);
    }
    return false;
  }
  ...
  async function beforeDelivery(card, { solicited = false } = {}) {
    if (!writable()) {
      return { deliver: false, state: 'owner-inactive', message: 'Osmosis is reconnecting its local owner.' };
    }
    if (!enabled()) {
      return { deliver: true };
    }
