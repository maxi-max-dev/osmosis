...
const { namespaceTreeNodes } = require('./project-concepts');
...
function createTreeService({ state, persistence, hub, provider, projectId = state?.project_id, canCommit = () => true }) {
  function writable() {
    try {
  ...
  async function ensureInitialTree(report) {
    ...
    const nodes = namespaceTreeNodes(projectId, validateTreeNodes(generated?.nodes));
    state.tree = {
      meta: {
        created_at: new Date().toISOString(),
        concept_namespace: projectId || null,
