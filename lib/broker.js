...
const { cardForClient, createPersistence, loadProjectState, snapshotFor } = require('./state');
...
function createBroker({
  ...
  async function initialize() {
    ...
    initialized = (async () => {
      ...
      for (const legacyProject of registry.listProjects()) {
        ...
        if (activationFor(legacyProject.project_id).state === 'activation-pending') {
          await settings.setProject(legacyProject.project_id, {
            carry: true,
      ...
      if ((legacySummary || legacyState) && activationFor(defaultProjectId).state === 'activation-pending') {
        await settings.setProject(defaultProjectId, {
          carry: true,
          capture_mode: 'agent-reports-only',
        });
      }
  ...
  async function makeChannel(summary) {
    ...
    if (channelConfig.mode === 'live') {
      ...
      const runStudioGeneration = (report) => new Promise((resolve) => {
        let settled = false;
        const settle = (result) => {
          if (!settled) {
            settled = true;
        ...
        if (closed || learningIsPaused()) {
          settle({ reason: closed ? 'broker-closed' : 'learning-paused', state: 'waiting' });
          return;
        }
        const accepted = scheduler.enqueue(summary.project_id, {
          ...
          async run() {
            if (closed || learningIsPaused()) {
              const paused = { reason: closed ? 'broker-closed' : 'learning-paused', state: 'waiting' };
              settle(paused);
              return paused;
            }
      ...
      // A process can restart with one or two already-durable candidates.
      // Hydration must resume that bounded work without waiting for a fresh
      // report to happen to arrive.
      queueMicrotask(() => {
        void studio.pump().catch((error) =>
          log('could not resume hydrated Studio work', error && error.message ? error.message : error),
        );
      });
    }
    ...
    const retainedStudioReportIds = state.studio?.candidates
      ?.flatMap((candidate) => candidate.report_ids || [candidate.report?.report_id])
      .filter(Boolean) || [];
    await ledger.reconcileDangling(summary.project_id, { retainReportIds: retainedStudioReportIds }).catch((error) =>
  ...
  async function updateSettings(patch) {
    ...
    if (keys.some((key) => !['global_learning', 'lesson_locale'].includes(key))) {
      throw new TypeError('Only global_learning and lesson_locale can be changed here.');
    }
  ...
  async function nextLesson(projectId, options = {}) {
    ...
    return result.card ? { ...result, card: cardForClient(result.card) } : result;
  }
  ...
  async function reviewLessons(projectId) {
    ...
    const review = channel.studio?.projection?.().review
      || snapshotFor(channel.state).cards.filter((card) => card?.state?.answered);
  ...
  async function initialEvents() {
    ...
    const legacy = current
      ? { cards: current.cards, strengths: current.strengths, tree: current.tree }
      : { cards: [], strengths: profileStore.strengths, tree: { meta: {}, nodes: [] } };
  ...
  async function reloadHydrated() {
    ...
    // A relay takeover can inherit a persisted Studio candidate just as a
    // cold start can. Resume it under the same fair global scheduler once
    // every hydrated state object has been refreshed.
    requestStudioWake();
  }
