...
const { createAnswerReceiptStore } = require('./answer-receipt-store');
...
const { createProjectWriteFence } = require('./project-write-fence');
...
function createBroker({
  ...
  // The server supplies its HTTP-port ownership predicate. Focused unit
  // callers remain an owner by default, but production writes cannot outlive
  // the listener that authorized this broker.
  canWrite = () => true,
} = {}) {
  if (!config || !hub) {
  ...
  const answerReceiptStore = createAnswerReceiptStore({ profileDir: config.profileDir });
  ...
  // Every broker instance is one HTTP owner's write generation. It lets
  // sibling persistence paths from this owner rebase safely while a prior
  // owner remains unable to overwrite the newer cards document.
  const cardsWriteEpoch = randomUUID();
  const projectWriteFences = new Map();
  ...
  function writeFenceFor(channelConfig) {
    const projectId = channelConfig?.projectId;
    if (typeof projectId !== 'string' || !projectId) {
      throw new TypeError('A project channel needs an id before it can claim a write fence.');
    }
    let fence = projectWriteFences.get(projectId);
    if (!fence) {
      fence = createProjectWriteFence({
        canWrite: () => !closed && canWrite(),
        ownerEpoch: cardsWriteEpoch,
        stateDir: channelConfig.stateDir,
      });
      projectWriteFences.set(projectId, fence);
    }
    return fence;
  }

  async function claimWriteFence(channelConfig) {
    const fence = writeFenceFor(channelConfig);
    if (!await fence.claim()) {
      const error = new Error('This broker no longer owns the project-state write lease.');
  ...
  async function makeChannel(summary) {
    ...
    // This is intentionally before the cards load. A fresh HTTP owner must
    // publish its epoch while the old cards revision is still unchanged, so a
    // paused predecessor cannot win the new owner's first-write window.
    const writeFence = await claimWriteFence(channelConfig);
    const state = await loadProjectState(channelConfig, profileStore.strengths);
    const persistence = createPersistence(channelConfig, {
      canCommit: () => writeFence.owns(),
      initialCardsRevision: state.cards_revision,
      profileStore,
      writeEpoch: cardsWriteEpoch,
    });
  ...
  async function acceptWarmupCandidate(projectId, observation) {
    ...
    const qualification = qualifyWarmupEvent({
      ...
      lesson_locale: activation.lesson_locale,
    });
    ...
    // only persists the selected catalog-localized warmup and paired source
    // candidate.
  ...
  async function reloadHydrated() {
    ...
    for (const summary of registry.listProjects()) {
      ...
      const hydrate = async () => {
        ...
        channel.persistence?.setCardsRevision?.(refreshed.cards_revision);
