...
function createReportPipeline({ cardService, config, curriculumService, hub, provider, replayService, state, ledger = null }) {
  const reports = [];
  ...
  async function createReportCard(report) {
    try {
      ...
      if (isMastered(state.strengths, generatedCard.concept_id)) {
        trace(report, 'refusal', 'suppressed', { reason: 'already-mastered', concept_id: generatedCard.concept_id });
        hub.broadcast('status', {
          state: 'skipped',
