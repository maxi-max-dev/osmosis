...
const { namespaceConceptId } = require('./project-concepts');
const { canonicalWarmupConceptId, catalogEntryForConceptId } = require('./warmup-catalog');

/**
 * Turn the catalog-selected warmup concept into a one-shot provider choice.
 *
 * A live project tree is intentionally grow-only and may not contain every
 * small, fixed warmup topic.  For an epoch follow-up we therefore offer the
 * provider exactly one synthetic allowed concept when no tree leaf aliases
 * the target.  Nothing is written into the tree: this is a scoped generation
 * constraint, and `markSurfaced()` remains a harmless no-op for the
 * synthetic concept after visible delivery.
 */
function warmupGenerationTarget({ curriculum, state, targetConceptId }) {
  const canonicalConceptId = canonicalWarmupConceptId(targetConceptId);
  if (!canonicalConceptId) {
    return null;
  }
  const available = Array.isArray(curriculum?.concepts) ? curriculum.concepts : [];
  const matchedConcept = available.find((concept) => (
    canonicalWarmupConceptId(concept?.concept_id) === canonicalConceptId
  ));
  const entry = catalogEntryForConceptId(canonicalConceptId);
  const providerConceptId = matchedConcept?.concept_id || canonicalConceptId;
  const persistedConceptId = matchedConcept
    ? curriculum?.conceptIdMap?.get(providerConceptId) || providerConceptId
    : namespaceConceptId(state?.project_id, canonicalConceptId);
  return {
    canonicalConceptId,
    concept: {
      concept_id: providerConceptId,
      concept_name: matchedConcept?.concept_name || entry?.title || canonicalConceptId,
      parent_id: matchedConcept?.parent_id ?? null,
    },
    persistedConceptId,
  };
}
...
function createReportPipeline({ cardService, config, curriculumService, hub, provider, replayService, state, ledger = null, studio = null }) {
   ...
   */
  async function generateForStudio(report, { canCommit = () => true, targetConceptId = null } = {}) {
    const writable = () => {
      try {
    ...
    try {
      ...
      // The local warmup and its true follow-up share an epoch *and* a
      // canonical catalog target.  Supplying only an epoch lets a provider
      // choose an unrelated tree leaf, which would honestly land in Next but
      // breaks the intended replacement.  Pin the provider to this one
      // allowed concept and validate it again after generation.
      const warmupTarget = warmupGenerationTarget({ curriculum, state, targetConceptId });
      const providerConcepts = warmupTarget ? [warmupTarget.concept] : curriculum.concepts || [];
      const conceptIdMap = warmupTarget
        ? new Map([
          ...(curriculum.conceptIdMap ? curriculum.conceptIdMap.entries() : []),
          [warmupTarget.concept.concept_id, warmupTarget.persistedConceptId],
        ])
        : curriculum.conceptIdMap;
      const allowedConceptIds = warmupTarget
        ? new Set([...(curriculum.conceptIds || []), warmupTarget.persistedConceptId])
      ...
      const providerCard = await provider.generateCard({
        ...
        masteredConceptIds: curriculum.masteredConceptIds || [],
        report,
        requiredConceptId: warmupTarget?.canonicalConceptId || null,
      });
      if (!writable()) {
        return { state: 'owner-inactive' };
      }
      const generatedCard = conceptIdMap?.has(providerCard.concept_id)
        ? { ...providerCard, concept_id: conceptIdMap.get(providerCard.concept_id) }
        : providerCard;
      ...
      if (warmupTarget && canonicalWarmupConceptId(generatedCard.concept_id) !== warmupTarget.canonicalConceptId) {
        throw new Error(`Provider did not return the required warmup concept: ${warmupTarget.canonicalConceptId}.`);
      }
      if (curriculum.usesProjectTree && !allowedConceptIds.has(generatedCard.concept_id)) {
        throw new Error(`Provider selected a concept outside the available tree leaves: ${generatedCard.concept_id}.`);
      }
