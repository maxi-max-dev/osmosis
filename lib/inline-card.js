...
function unansweredCards(state) {
  const hiddenReadyId = state?.studio?.ready_card_id;
  return Array.isArray(state?.cards)
    ? state.cards.filter((card) => card && !card.state?.answered && card.card_id !== hiddenReadyId)
    : [];
}
...
function renderInlineCard({ state = {}, answerUrl = DEFAULT_ANSWER_URL, refreshUrl = DEFAULT_REFRESH_URL } = {}) {
      ...
      <main class="shell">
        <section class="card" data-osmosis-inline-card="pending" aria-label="Osmosis inline lesson">
          ...
          <div class="empty">
            ...
            <small>Keep building. Your next observed activity or agent report will become a focused lesson, while mastered knowledge stays carried over.</small>
          </div>
  ...
  const sourceTask = observed
    ...
    : text(source.task, 'Milestone');
  const sourceWhat = observed
    ? sourceKind === 'observed-change'
      ? 'Osmosis observed a local Codex change.'
      : 'Osmosis observed local Codex activity.'
    : text(source.what_i_did, 'Your agent reported a milestone.');
