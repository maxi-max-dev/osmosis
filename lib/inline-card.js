...
function renderInlineCard({ state = {}, answerUrl = DEFAULT_ANSWER_URL, refreshUrl = DEFAULT_REFRESH_URL } = {}) {
      ...
      <main class="shell">
        <section class="card" data-osmosis-inline-card="pending" aria-label="Osmosis inline lesson">
          ...
          <div class="empty">
            ...
            <small>Keep building. Your next local change or agent report will become a focused lesson, while mastered knowledge stays carried over.</small>
          </div>
  ...
  const sourceKind = source.kind === 'observed' ? 'observed' : 'agent';
  const sourceLabel = sourceKind === 'observed' ? 'Observed change' : 'Reported by agent';
  const sourceTask = text(source.task, sourceKind === 'observed' ? 'Local Codex activity' : 'Milestone');
  const sourceWhat = text(
    source.what_i_did,
    sourceKind === 'observed' ? 'Osmosis observed a local Codex change.' : 'Your agent reported a milestone.',
  );
