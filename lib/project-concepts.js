'use strict';

// Keep this tiny normalizer local rather than importing `concepts`: that
// module reads mastery, and mastery needs these namespace helpers. Avoiding
// the cycle keeps legacy mastery reads reliable during process startup.
function normalizeConceptId(value) {
  if (typeof value !== 'string') {
    return '';
  }
  return value
    .normalize('NFKD')
    .replace(/[\u0300-\u036f]/g, '')
    .toLowerCase()
    .replace(/[^a-z0-9]+/g, '-')
    .replace(/^-+|-+$/g, '');
}

// Template curriculum is intentionally shared. It teaches how Osmosis itself
// works, rather than a fact discovered from one particular project.
const GLOBAL_CONCEPT_IDS = new Set(['feedback-loop']);

function isGlobalConceptId(conceptId) {
  return GLOBAL_CONCEPT_IDS.has(conceptId);
}

function splitNamespacedConceptId(conceptId) {
  if (typeof conceptId !== 'string') {
    return null;
  }
  const separator = conceptId.indexOf(':');
  if (separator <= 0 || separator === conceptId.length - 1 || conceptId.indexOf(':', separator + 1) !== -1) {
    return null;
  }
  const projectId = conceptId.slice(0, separator);
  const localId = conceptId.slice(separator + 1);
  if (normalizeConceptId(projectId) !== projectId || normalizeConceptId(localId) !== localId) {
    return null;
  }
  return { localId, projectId };
}

function localConceptId(conceptId) {
  return splitNamespacedConceptId(conceptId)?.localId || conceptId;
}

function namespaceConceptId(projectId, conceptId) {
  const localId = normalizeConceptId(localConceptId(conceptId));
  if (!localId) {
    return '';
  }
  if (isGlobalConceptId(localId)) {
    return localId;
  }
  const normalizedProjectId = normalizeConceptId(projectId);
  return normalizedProjectId ? `${normalizedProjectId}:${localId}` : localId;
}

function namespaceTreeNodes(projectId, nodes) {
  const idMap = new Map();
  for (const node of nodes) {
    idMap.set(node.concept_id, namespaceConceptId(projectId, node.concept_id));
  }
  return nodes.map((node) => ({
    ...node,
    concept_id: idMap.get(node.concept_id),
    parent_id: node.parent_id === null ? null : idMap.get(node.parent_id),
  }));
}

// Trees written before the broker introduced per-project concept ids used
// their provider's local ids directly.  Migrate those trees at channel
// hydration rather than waiting for the next generation: otherwise the
// selectable-concept map would hand a new provider an unsalted id again.
//
// This deliberately leaves the profile alone. `mastery.strengthFor()` has a
// legacy read path from a newly scoped id to an old unsalted profile entry, so
// previously earned progress remains visible without turning one project's
// old provider concepts into another project's write target.
function migrateTreeConceptNamespace(projectId, tree) {
  if (!projectId || !Array.isArray(tree?.nodes) || tree.nodes.length === 0) {
    return { migrated: false, tree };
  }

  const idMap = new Map();
  for (const node of tree.nodes) {
    if (node && typeof node.concept_id === 'string') {
      idMap.set(node.concept_id, namespaceConceptId(projectId, node.concept_id));
    }
  }

  function remap(conceptId) {
    if (typeof conceptId !== 'string') {
      return conceptId;
    }
    return idMap.get(conceptId) || namespaceConceptId(projectId, conceptId);
  }

  const nodes = tree.nodes.map((node) => {
    if (!node || typeof node !== 'object') {
      return node;
    }
    return {
      ...node,
      concept_id: remap(node.concept_id),
      parent_id: node.parent_id === null ? null : remap(node.parent_id),
    };
  });
  const originalMeta = tree.meta && typeof tree.meta === 'object' && !Array.isArray(tree.meta) ? tree.meta : {};
  const surfaced = Array.isArray(originalMeta.surfaced_concept_ids)
    ? originalMeta.surfaced_concept_ids.map(remap)
    : originalMeta.surfaced_concept_ids;
  const meta = {
    ...originalMeta,
    concept_namespace: projectId,
    ...(Array.isArray(originalMeta.surfaced_concept_ids) ? { surfaced_concept_ids: surfaced } : {}),
  };

  const nodesChanged = nodes.some((node, index) => {
    const previous = tree.nodes[index];
    return node?.concept_id !== previous?.concept_id || node?.parent_id !== previous?.parent_id;
  });
  const surfacedChanged = Array.isArray(originalMeta.surfaced_concept_ids) && surfaced.some(
    (conceptId, index) => conceptId !== originalMeta.surfaced_concept_ids[index],
  );
  const metaChanged = originalMeta !== tree.meta || originalMeta.concept_namespace !== projectId || surfacedChanged;

  if (!nodesChanged && !metaChanged) {
    return { migrated: false, tree };
  }

  return {
    migrated: true,
    tree: {
      ...tree,
      meta,
      nodes,
    },
  };
}

function validProviderConceptId(conceptId) {
  if (typeof conceptId !== 'string' || !conceptId) {
    return false;
  }
  const namespaced = splitNamespacedConceptId(conceptId);
  return Boolean(namespaced || normalizeConceptId(conceptId) === conceptId);
}

module.exports = {
  GLOBAL_CONCEPT_IDS,
  isGlobalConceptId,
  localConceptId,
  migrateTreeConceptNamespace,
  namespaceConceptId,
  namespaceTreeNodes,
  splitNamespacedConceptId,
  validProviderConceptId,
};
