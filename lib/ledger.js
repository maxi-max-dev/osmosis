...
function createLedger({ profileDir, ledgerDir, maxBytes = 512 * 1024, ringSize = 200, now = defaultNow } = {}) {
  ...
  async function reconcileDangling(projectId, { retainReportIds = [] } = {}) {
    ...
    for (const entry of byReport.values()) {
      // Studio candidates are durable work, not a crashed delivery. Their
      // report ids survive in cards.json and will be resumed by the channel
      // after hydration, so the activity drawer must not falsely call them
      // "lost at restart".
      if (entry.report_id && retained.has(entry.report_id)) {
        continue;
      }
