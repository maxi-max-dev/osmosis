...
function createLedger({ profileDir, ledgerDir, maxBytes = 512 * 1024, ringSize = 200, now = defaultNow } = {}) {
  ...
  const ring = new Map();
  // Outbox IDs are stable across owner restarts. Keep a per-project set that
  // is hydrated from both retained JSONL generations on first use so a
  // partially flushed transition can be replayed without duplicating trace
  // entries after takeover.
  const knownOutboxIds = new Map();
  ...
  async function outboxIdsFor(projectId) {
    const remembered = knownOutboxIds.get(projectId);
    if (remembered) {
      return remembered;
    }
    const ids = new Set(
      [
        ...(await parseJsonLines(rotationPathFor(projectId))),
        ...(await parseJsonLines(filePathFor(projectId))),
      ]
        .map((entry) => entry?.outbox_id)
        .filter((id) => typeof id === 'string' && id),
    );
    knownOutboxIds.set(projectId, ids);
    return ids;
  }
  ...
  function append(projectId, entry) {
    ...
    return enqueue(async () => {
      const outboxIds = normalized.outbox_id ? await outboxIdsFor(normalized.project_id) : null;
      if (outboxIds?.has(normalized.outbox_id)) {
      ...
      outboxIds?.add(normalized.outbox_id);
      remember(normalized);
      return { ...normalized };
    });
  }
  ...
  function forProject(projectId) {
    ...
    return {
      ...
      appendIdempotent: (entry) => append(id, entry),
