...
// UI language is intentionally independent from lesson generation language.
// Existing cards are historical learning artefacts and are never rewritten
// merely because the surrounding Studio interface changes language.
const UI_LOCALE_ENGLISH = 'en';
const UI_LOCALE_SIMPLIFIED_CHINESE = 'zh-CN';
...
const UI_LOCALE_VALUES = new Set([UI_LOCALE_ENGLISH, UI_LOCALE_SIMPLIFIED_CHINESE]);
...
function defaultSettings() {
  return {
    ...
    // Chinese is the deliberate default for a new local Studio.  A v1 file
    // is migrated below by copying its former single locale once, then the
    // two preferences remain fully separate.
...
function normalizedUiLocale(value) {
  return UI_LOCALE_VALUES.has(value) ? value : UI_LOCALE_SIMPLIFIED_CHINESE;
}
...
function normalizeSettings(value) {
  ...
  // A v1 preferences document used lesson_locale for the entire surface.
  // Copy that value exactly once at read/next-write time; do not keep the two
  // fields coupled afterwards. A newly created v2 document starts Chinese.
  const legacyUiLocale = Object.hasOwn(value, 'ui_locale')
    ? normalizedUiLocale(value.ui_locale)
    : Object.hasOwn(value, 'lesson_locale')
      ? normalizedUiLocale(value.lesson_locale)
      : fallback.ui_locale;
  return {
    version: 2,
    ...
    ui_locale: legacyUiLocale,
    mascot_enabled: value.mascot_enabled !== false,
    local_conversation_titles: value.local_conversation_titles === true,
...
function createSettingsStore({ profileDir, settingsPath, fsApi = fs } = {}) {
  ...
  async function setUiLocale(uiLocale) {
    if (!UI_LOCALE_VALUES.has(uiLocale)) {
      throw new TypeError('UI locale must be "en" or "zh-CN".');
    }
    return update((current) => ({ ...current, ui_locale: uiLocale }));
  }

  async function setMascotEnabled(enabled) {
    if (typeof enabled !== 'boolean') {
      throw new TypeError('Mascot enabled must be boolean.');
    }
    return update((current) => ({ ...current, mascot_enabled: enabled }));
  }

  async function setLocalConversationTitles(enabled) {
    if (typeof enabled !== 'boolean') {
      throw new TypeError('Local conversation titles must be boolean.');
    }
    return update((current) => ({ ...current, local_conversation_titles: enabled }));
  }
