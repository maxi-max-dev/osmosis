...
function createStudioService({
  ...
  async function persistGenerated(card, placement) {
    ...
    // A ready card lives only in the Studio buffer until the learner asks for
    // it. Storing it in the compatibility/history array before that moment
    // was the second bookkeeping path that allowed a reload to orphan it.
    if (placement === 'current' && !state.cards.some((item) => item?.card_id === card.card_id)) {
  ...
  async function next({ auto = false, enabled = true, interaction_token: token } = {}) {
    ...
    if (!state.cards.some((card) => card?.card_id === nextCard.card_id)) {
   ...
   */
  async function reload(applyHydratedState) {
