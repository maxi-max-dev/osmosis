...
const { idlePresentation, projectLedgerEntries, transitionPresentation } = require('./presentation-projection');
...
function createStudioService({
  ...
  function setObservedProgress(candidate, reason = 'warmup-eligible') {
    ...
    state.studio.presentation = transitionPresentation(state.studio.presentation, {
      epochId: candidate.activity_epoch_id || observationId,
      phase: 'observed',
      reason,
  ...
  async function commitOutboxTransition(entries, { afterPersist = null, afterLedger = null, broadcastCard = null } = {}) {
    ...
    // The strip reads this projection only; it is written in the same atomic
    // state+outbox step as the event it represents, then published after the
    // idempotent ledger flush just like every other Studio transition.
    state.studio.presentation = projectLedgerEntries(
      state.studio.presentation,
      entries,
  ...
  return {
    ...
    answerWarmup,
    afterAnswer: (card) => {
      if (state.studio.presentation?.phase === 'card-ready') {
        state.studio.presentation = idlePresentation(new Date(clock()).toISOString());
        void persist().then(() => emit()).catch(() => {});
      } else {
        emit();
      }
      return status();
