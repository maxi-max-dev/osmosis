...
// A dormant rollout can become active again after the discovery window. Keep
// a small identity-keyed cursor cache so that resuming it never replays its
// already-consumed JSONL history. This is deliberately bounded: a watcher
// must not retain an unbounded catalogue of finished Codex sessions.
const DEFAULT_MAX_CURSOR_TOMBSTONES = 64;
...
function limitsFor(config) {
  return {
    ...
    maxCursorTombstones: positiveLimit(config?.ambientMaxCursorTombstones, DEFAULT_MAX_CURSOR_TOMBSTONES),
...
function makeReportFromSignals(signals) {
...
function makeReport(state) {
  return reportFromSignalsWithConversation(state, [...state.pending.values()]);
}

function reportFromSignalsWithConversation(state, signals) {
  const report = makeReportFromSignals(signals);
  if (state?.conversation?.session_id) {
    // This private bridge exists only between the ambient reader and the
    // owner broker, where it is immediately HMACed and stripped. Never put
    // it in a public report schema or a durable report collection.
    report.__osmosis_local_conversation = { ...state.conversation };
  }
  return report;
}

function hashObservation(value) {
  return createHash('sha256').update(value).digest('hex').slice(0, 48);
}

function rolloutIdentityForEvent(state, event) {
  const sessionId = sessionIdFromEvent(event);
  // The raw session id never enters a report, card, ledger message, or UI.
  // Its hash is a stable local routing key for bounded warmup dedupe.
  return hashObservation(sessionId || state.identity || 'unknown-rollout');
}

function sessionIdFromEvent(event) {
  const payload = event?.payload && typeof event.payload === 'object' ? event.payload : {};
  // `payload.id` belongs to the session_meta record only. Later response
  // items have their own message/tool ids and must not fracture one local
  // conversation into many opaque identities.
  return shortText(
    payload.session_id || event?.session_id || (event?.type === 'session_meta' ? payload.id : ''),
    '',
    256,
  );
}
...
function conversationFromEvent(event) {
  const payload = event?.payload && typeof event.payload === 'object' ? event.payload : {};
  const sessionId = sessionIdFromEvent(event);
  // Codex rollouts provide the local prompt in `event_msg/user_message`.
  // Read it only long enough to let the opt-in local title store sanitize it;
  // this raw text never becomes report, card, ledger, replay, or MCP data.
  const title = event?.type === 'event_msg' && payload.type === 'user_message'
    ? shortText(payload.message, '', 4_096)
    : '';
  return { sessionId, title };
}

function observationIdForEvent(state, event) {
  return hashObservation(`${rolloutIdentityForEvent(state, event)}:${JSON.stringify(event).slice(0, MAX_EVENT_TEXT_BYTES)}`);
}
...
function createAmbientWatcher({
  ...
  // Tombstones are keyed by inode identity rather than pathname. A rollout
  // can be renamed while dormant; its byte cursor remains meaningful only for
  // that same file identity.
  const cursorTombstones = new Map();
  ...
  function routeFromResolved(value, canonicalCwd) {
    ...
    if (value.root || value.registered === false) {
      ...
      return {
        key: `unregistered:${value.root || canonicalCwd}`,
        registered: false,
        root: value.root || canonicalCwd,
        ...(shortText(value.reason, '', 96) ? { reason: shortText(value.reason, '', 96) } : {}),
      };
    }
    return null;
  }
  ...
  async function notifyWarmupCandidate(state, event, route, signals) {
    if (typeof onWarmupCandidate !== 'function' || !route || signals.length === 0) {
      return { handled: false };
    }
    const observationId = observationIdForEvent(state, event);
    try {
      const observation = {
        activity_epoch_id: observationId,
        event,
        observation_id: observationId,
        report: reportFromSignalsWithConversation(state, signals),
        rollout_identity: rolloutIdentityForEvent(state, event),
        route: {
          ...(route.project_id ? { project_id: route.project_id } : {}),
          reason: route.reason || (route.disabled ? 'learning-paused' : route.registered ? '' : 'project-unregistered'),
          registered: route.registered === true,
        },
      };
      // Match the existing onReport callback shape without making the raw
      // cwd part of the callback contract. The broker independently checks
      // this id against observation.route before it writes anything.
      const result = await onWarmupCandidate(
        observation,
        route.registered === true && typeof route.project_id === 'string'
          ? { project_id: route.project_id }
          : undefined,
      );
      return result && typeof result === 'object' ? result : { handled: false };
    } catch (error) {
      safeLog(log, 'ambient watcher could not prepare a warmup candidate', error);
      return { handled: false };
    }
  }
  ...
  async function processEvent(state, event, budget) {
    ...
    if (conversation.sessionId) {
      state.conversation = {
        ...
        ...(conversation.title ? { title: conversation.title } : state.conversation?.title ? { title: state.conversation.title } : {}),
      };
    } else if (conversation.title && state.conversation?.session_id) {
      state.conversation = { ...state.conversation, title: conversation.title };
    }
    const signals = signalsFromEvent(event);
    const details = execDetails(event?.payload);
    const eventWorkdir = workdirFromEvent(event);
    let resolvedRoute = state.route;
    if (details) {
      ...
      resolvedRoute = await updateSessionCwd(state, eventWorkdir);
    }
    if (resolvedRoute?.disabled) {
      await notifyWarmupCandidate(state, event, resolvedRoute, signals);
      return;
    }
    if (!state.route) {
      return;
    }
    const fastPath = await notifyWarmupCandidate(state, event, state.route, signals);
    if (fastPath.handled) {
      return;
    }
  ...
  function hasCompleteCursorBoundary(state) {
    // A cursor alone cannot reconstruct an incomplete JSONL record. Retain
    // such a state until it is completed, replaced, or truncated rather than
    // dropping its partial bytes and later parsing the tail as a new line.
    return Boolean(
      state
      && state.trailing.length === 0
      && !state.discardUntilNewline,
    );
  }

  function canTombstoneCursor(state) {
    return Boolean(
      hasCompleteCursorBoundary(state)
      && state.identity
  ...
  function routeSnapshotForTombstone(state) {
    // A dormant cursor needs the last trusted session route as well as its
    // byte offset: patch/MCP rollout events do not carry a workdir of their
    // own. Keep only the canonical cwd and stable project id, then validate
    // both again before a resumed file can emit anything.
    const canonicalCwd = shortText(state?.workdir, '', 2_048);
    if (!canonicalCwd) {
      return null;
    }
    const projectId = state?.route?.registered
      ? shortText(state.route.project_id, '', 256)
      : '';
    return {
  ...
  function unknownRoute(canonicalCwd, reason = 'unregistered-project') {
    return {
      key: `unregistered:${canonicalCwd}`,
      reason,
      registered: false,
      root: canonicalCwd,
    };
  ...
  async function restoreTombstoneRoute(state, tombstone) {
    const snapshot = tombstone?.route;
    const canonicalCwd = shortText(snapshot?.canonical_cwd, '', 2_048);
    if (!canonicalCwd || state.ignoreAmbient) {
      return;
    }

    // `workdir` is already canonical because it was learned through an exec
    // event. Never trust the saved project id by itself: a carried project
    // must still resolve to the same broker channel at resume time.
    state.workdir = canonicalCwd;
    const route = await resolveRoute(canonicalCwd);
    if (route?.disabled) {
      state.route = null;
      return;
    }
    ...
    if (!route) {
      state.route = unknownRoute(canonicalCwd);
      return;
    }

    const expectedProjectId = shortText(snapshot.project_id, '', 256);
    if (
      expectedProjectId
      && (!route?.registered || route.project_id !== expectedProjectId)
    ) {
      // A route that was unregistered, uncarried, or reassigned while this
      // rollout was idle must not inherit the old project's authority. Keep
      // an unknown route so the normal suppression ledger path remains
      // honest instead of silently accepting or dropping the event.
      state.route = unknownRoute(canonicalCwd, route?.reason || 'unregistered-project');
      return;
    }
    state.route = route;
  }

  function rememberCursor(state) {
    if (!canTombstoneCursor(state)) {
      return;
    }
    // Re-inserting makes this the newest dormant cursor even if an inode was
    // observed again after a rename. Map insertion order evicts the oldest.
    cursorTombstones.delete(state.identity);
    cursorTombstones.set(state.identity, {
      ignoreAmbient: state.ignoreAmbient,
      offset: state.offset,
      route: routeSnapshotForTombstone(state),
    });
    while (cursorTombstones.size > limits.maxCursorTombstones) {
      const oldestIdentity = cursorTombstones.keys().next().value;
      cursorTombstones.delete(oldestIdentity);
    }
  }

  function takeCursor(identity) {
    if (!identity) {
      return null;
    }
    const tombstone = cursorTombstones.get(identity) || null;
    if (tombstone) {
      cursorTombstones.delete(identity);
    }
    return tombstone;
  }
  ...
  async function processFile(filePath, stats, timestamp, budget, initialScan, byteLimit = budget.remainingBytes) {
    ...
    if (!state) {
      ...
      // Startup EOF baselines always win: only a cursor created during this
      // watcher lifetime is eligible to resume an inactive rollout.
      const tombstone = baseline ? null : takeCursor(identity);
      state = createFileState({
        offset: baseline ? baseline.offset : tombstone ? tombstone.offset : initialScan && !startupBaseline ? stats.size : 0,
        identity: baseline?.identity || identity,
      });
      state.ignoreAmbient = Boolean(tombstone?.ignoreAmbient);
      await restoreTombstoneRoute(state, tombstone);
  ...
  function getDebugState() {
    return {
      cursorTombstones: cursorTombstones.size,
