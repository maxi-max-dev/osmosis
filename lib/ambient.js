...
function makeReport(state) {
  return makeReportFromSignals([...state.pending.values()]);
}
...
function rolloutIdentityForEvent(state, event) {
  const payload = event?.payload && typeof event.payload === 'object' ? event.payload : {};
  const sessionId = shortText(payload.session_id || event?.session_id, '', 256);
  ...
  async function notifyWarmupCandidate(state, event, route, signals) {
    ...
    try {
      const observation = {
        ...
        report: makeReportFromSignals(signals),
