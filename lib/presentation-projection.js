...
function projectLedgerEntries(current, entries, updatedAt = new Date().toISOString()) {
  ...
  for (const entry of Array.isArray(entries) ? entries : []) {
    ...
    // warmup_served or provider-result as ready makes an atomic observed+
    // warmup transition visibly jump to ready, then regress to preparing as
    // its paired generation begins. A durable next-ready buffer, however, is
    // a real terminal learner-facing outcome: the Studio snapshot exposes its
    // Next control and it must not leave the strip spinning forever. This is
    // presentation only—the ledger correctly remains buffered/waiting until
    // the learner promotes or suppresses that card.
    const readyForLearner = ['delivery', 'promotion', 'warmup_replaced'].includes(entry.event)
      || (entry.event === 'buffered' && entry.reason === 'next-ready');
    if (readyForLearner) {
      presentation = transitionPresentation(presentation, {
        epochId,
        phase: 'card-ready',
