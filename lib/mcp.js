...
function createMcpServer({
  ...
  function normalizedInlineCardHtml(html) {
  ...
  function handle(request) {
    ...
    if (method === 'resources/read') {
      ...
      const sendResource = (html) => result(id, {
          contents: [
            {
              uri: INLINE_CARD_URI,
              mimeType: 'text/html',
      ...
      const html = inlineCardHtml();
      if (html && typeof html.then === 'function') {
        void html.then(sendResource);
      } else {
        sendResource(html);
      }
      return;
